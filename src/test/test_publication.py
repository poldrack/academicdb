...
def article_dict():
    ...
    'scopus_coauthor_ids': ['57190392076',
    '56608578700',
    '56559386700',
    '57204965665',
    '57223889578',
...
def test_latex_reference_article(article_dict):
    ...
    assert ref == 'Ciric R, Thompson WH, Lorenz R, Goncalves M, MacNicol EE, Markiewicz CJ, Halchenko YO, Ghosh SS, Gorgolewski KJ, Poldrack RA, Esteban O (2022). TemplateFlow: FAIR-sharing of multi-scale, multi-species brain models. \\textit{Nature Methods, 19}, 1568-1571.'

def test_md_reference_article(article_dict):
    pub = JournalArticle()
    pub.from_dict(article_dict)
    ref = pub.format_reference(format='md', etalthresh=100)
    assert ref == 'Ciric R, Thompson WH, Lorenz R, Goncalves M, MacNicol EE, Markiewicz CJ, Halchenko YO, Ghosh SS, Gorgolewski KJ, Poldrack RA, Esteban O (2022). TemplateFlow: FAIR-sharing of multi-scale, multi-species brain models. *Nature Methods, 19*, 1568-1571.'
...
def test_latex_reference_book(book_dict):
    ...
    assert ref == 'Poldrack RA, Mumford JA, Nichols TE (2011). \\textit{Handbook of Functional MRI Data Analysis}. Cambridge: Cambridge University Press.'

def test_md_reference_book(book_dict):
    pub = Book()
    pub.from_dict(book_dict)
    ref = pub.format_reference(format='md')
    assert ref == 'Poldrack RA, Mumford JA, Nichols TE (2011). *Handbook of Functional MRI Data Analysis*. Cambridge: Cambridge University Press.'
